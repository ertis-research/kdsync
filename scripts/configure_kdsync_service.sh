...
set -e # Exit on eror
set -x # Show arguments exe

mkdir -p /etc/derecho
cat > /etc/derecho/derecho.cfg <<EOF
[DERECHO]
# leader ip - the leader's ip address
leader_ip = 192.168.44.19
...
# rdmc tcp port
rdmc_port = 31675
# external tcp port listening to external clients
external_port = 32645
...
EOF
nano /etc/derecho/derecho.cfg


mkdir -p /etc/kdsync
cat > /etc/kdsync/kdsync_args <<EOF
INSTANCES=3
BROKERS=127.0.0.1:9092
TOPIC=test
EOF
nano /etc/kdsync/kdsync_args


cat > /etc/systemd/system/kdsync.service <<'EOF'
[Unit]
...
Type=simple
User=root
Environment="DERECHO_CONF_FILE=/etc/derecho/derecho.cfg"
EnvironmentFile=/etc/kdsync/kdsync_args
ExecStart=/usr/local/bin/kdsync $INSTANCES $BROKERS $TOPIC
Restart=on-failure
RestartSec=5s
...
systemctl enable kdsync
systemctl start kdsync